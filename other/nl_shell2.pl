...
noun(p,M)			--> [Noun_p], {pred2gr(_P,1,n/Noun,M),atom_concat(Noun,s,Noun_p)}.
iverb(s,M)			--> [Verb_s], {pred2gr(_P,1,v/Verb,M),atom_concat(Verb,s,Verb_s)}.
...
pred(male,    1,[a/male,n/man]).
pred(female,  1,[a/female,n/woman]).
...
show_answer(Answer):-
	...
	; Answer=no            -> writes(['I am unable to answer that question.'])
	; Answer=fail          -> writes(['I am unable to parse that.'])
	; Answer=proof(no)     -> writes(['I am unable to answer that question.'])
	; Answer=explain(_,no) -> writes(['I am unable to answer that question.'])
	; Answer=answers([])   -> writes(['No'])
	; Answer=explain(Q,P)  -> reverse(P,RP),write_exp(Q,RP)
